...
class StateAgent(BaseAgent):
    ...
    async def _classify_entities_with_llm(
        self, entities: list[str], logic_model: SquareLogicModel
...
"""
    ...
    def _heuristic_classify_entities(
        ...
        for entity in entities:
            ...
            for suffix in adjective_suffixes:
                if (
                    entity_lower.endswith(suffix)
                    and len(entity_lower) > len(suffix) + 2
                ):
                    is_attribute = True
            ...
            if not is_attribute:
                ...
                appears_as_subject = sum(
                    1 for r in logic_model.relations if r.subject == entity
                )
    ...
    async def _convert_logic_to_states(
        ...
        if not state_names:
            ...
            ) = await self._extract_states_and_attributes(logic_model)
        ...
        if (
            requirements
            and "transitions" in requirements
            and requirements["transitions"]
        ):
            for trans_def in requirements["transitions"]:
                ...
                if from_state and to_state:
                    transition = StateTransition(
                        ...
                        to_state=to_state,
                        condition=condition
                        or f"Transition from {from_state} to {to_state}",
        ...
        if not has_explicit_transitions:
            ...
            for relation in logic_model.relations:
                ...
                if subject not in state_names_set or predicate not in state_names_set:
                    continue

                # Skip if either entity is classified as an attribute
                subject_is_attribute = (
                    classifications.get(subject, {}).get("type") == "attribute"
                )
                predicate_is_attribute = (
                    classifications.get(predicate, {}).get("type") == "attribute"
                )
                ...
                if relation.relation_type == LogicRelationType.A:
                    ...
                    transition = StateTransition(
                        ...
                        confidence=relation.confidence
                        * 0.5,  # Lower confidence for partial relations
                    )
                    state_machine.add_transition(transition)
